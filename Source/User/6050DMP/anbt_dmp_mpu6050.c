...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("P-2,Finish DMP set sensor.");		//完成传感去设定
	else printf("P-2,DMP set sensor error.");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("P-3,Finish DMP configure fifo.");		//完成fifo设置
	else printf("P-3,DMP configure fifo error.");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ)) printf("P-4,Finish DMP set sample rate.");			//完成DMP采样率设置
	else printf("P-4,DMP set sample_rate error.");
	//
	if(!dmp_load_motion_driver_firmware())	printf("P-5,Finish DMP load motion driver firmware.");				//开启DMP功能
	else printf("P-5,DMP load motion driver firmware error.");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) printf("P-6,Finish DMP set orientation.");		//初始方向设定偏差
	...
	else printf("P-9,DMP set dmp state error.");					//Dmp启动失败
	//
	return 0;
}
